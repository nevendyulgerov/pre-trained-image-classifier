...
def print_results(results_dic, results_stats_dic, model, 
    ...
    print('Number of "Non-a" Dog Images: {}'.format(results_stats_dic['n_notdogs_img']))
    
    for key, value in results_stats_dic.items():
          if key.startswith('ptc'):
    ...
    if print_incorrect_dogs:
        if results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images']:
            for image, labels in results_dic.items():
                if sum(labels[3:]) == 1:
                    print('Incorrectly classified image: {}. Labels: {}'.format(image, labels))
        ...
        if results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed']:
            for image, labels in results_dic.items():
                if sum(labels[3:]) == 2 and labels[2] == 0:
                    print('Incorrectly classified breed: {}, Labels: {}'.format(image, labels))
